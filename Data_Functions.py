...
def Get_MI_Matrices(df_orig, Features, target, classif=True):
    assert df_orig[target].dtype == "bool", (
        "Error in Get_MI_Matrices: Expecting a boolean binary target"
    )
    ...
    MI_matrix_T = pd.DataFrame(
        np.zeros((len(Features), len(Features))), index=Features, columns=Features
    )
    MI_matrix_NT = pd.DataFrame(
        np.zeros((len(Features), len(Features))), index=Features, columns=Features
    )
    ...
    MI_max = max(
        MI_matrix.values.max(), MI_matrix_T.values.max(), MI_matrix_NT.values.max()
    )
    MI_min = 0
...
def Extract_PassengerId_Info(df):
    df2 = df.copy()
    df2_split = df2.PassengerId.str.split("_", expand=True).rename(
        {0: "GroupId", 1: "ppId"}, axis=1
    )
    df2 = pd.concat([df2, df2_split[["GroupId", "ppId"]]], axis=1)
    if df2_split.shape[1] > 2:
        mask = df2_split[2].isnull()
        df2["GroupSize"] = df2[mask].groupby("GroupId")["ppId"].transform(len)
        df2.GroupSize = df2.groupby("GroupId")["GroupSize"].transform("max").astype(int)
    else:
    ...
    df2_split = (
        df2.GroupId.str.extractall("(.)")[0]
        .unstack()
        .rename({0: "G1", 1: "G2", 2: "G3", 3: "G4"}, axis=1)
    )
...
def Extract_Initial_Data_orig(df, version=1):
    df = Extract_PassengerId_Info(df)
    df_split = df.Cabin.str.split("/", expand=True).rename(
        {0: "deck", 1: "Cabin Number", 2: "side"}, axis=1
    )
    df = pd.concat([df, df_split], axis=1)
    df_split = df.Name.str.split(" ", expand=True).rename(
        {0: "First Name", 1: "Last Name"}, axis=1
    )
    df = pd.concat([df, df_split], axis=1)
    ...
    if version == 3:
        is_extra = df.PassengerId.astype(str).str.endswith("_x")
        base = df.loc[~is_extra].copy()
        extra = df.loc[is_extra].copy()
        base["FamilySize"] = base.groupby("Last Name")["Last Name"].transform(len)
        base["FirstNameLength"] = (
            base["First Name"].fillna("").astype(str).apply(len).replace(0, np.nan)
        )
        base["LastNameLength"] = (
            base["Last Name"].fillna("").astype(str).apply(len).replace(0, np.nan)
        )
        base["GroupFamilySize"] = base.groupby(["Last Name", "GroupId"])[
            "Last Name"
        ].transform(len)
        base["CabinFamilySize"] = base.groupby(["Last Name", "Cabin"])[
            "Cabin"
        ].transform(len)
        base["CabinGroupSize"] = base.groupby(["GroupId", "Cabin"])["Cabin"].transform(
            len
        )
        base["CabinSize"] = base.groupby("Cabin")["Cabin"].transform(len)
        ...
        lookup = base.set_index("PassengerId")[cols]

        extra["base_pid"] = extra.PassengerId.str.replace(r"_x$", "", regex=True)
        for c in cols:
            extra[c] = extra.base_pid.map(lookup[c])

        extra.drop(columns=["base_pid"], inplace=True)
        out = pd.concat([base, extra], axis=0, ignore_index=True)
        out.set_index("PassengerId", inplace=True)
        return out

    df["FamilySize"] = df.groupby("Last Name")["Last Name"].transform(len)
    df["FirstNameLength"] = (
        df["First Name"].fillna("").astype(str).apply(len).replace(0, np.nan)
    )
    df["LastNameLength"] = (
        df["Last Name"].fillna("").astype(str).apply(len).replace(0, np.nan)
    )
    df["GroupFamilySize"] = df.groupby(["Last Name", "GroupId"])["Last Name"].transform(
        len
    )
...
def Extract_Initial_Data(
    ...
    df_all = Extract_Initial_Data_orig(df_all, version)
    ...
    if version == 1:
        ...
        df_all["Age_LifeStages_lbs"] = pd.cut(
            df_all.Age,
            bins=bin_edges,
            labels=LifeStages_choices,
            right=False,
            include_lowest=True,
        )
        df_all["Age_LifeStages"] = df_all.Age_LifeStages_lbs.cat.codes.replace(
            -1, np.nan
        )
        df_all.drop(
...
def Clean_and_dropna(df_orig, df_test_orig):
    ...
    df_test = (df_clean.iloc[-len(df_test) :]).drop(
        columns=["Transported", "Last Name"]
    )
...
def Clean_and_preImpute(df_orig, df_test_orig, verbose=True):
    ...
    if verbose:
        print(
            f"Number of null rows before cleaning: {len(df_orig[df_orig.isnull().any(axis=1)])} in training data\
    ...
    if verbose:
        null_mask = df.isnull().any(axis=1)
        print(
            f"There are {len(df[null_mask])} remaining null rows in training data after cleaning"
        )
...
def Pre_Impute_Features(df_orig):
    ...
    df.loc[mask, "Destination_PSO J318.5-22"] = df.loc[
        mask, "Destination_PSO J318.5-22"
    ].fillna(0)
    ...
    def Family_to_planet(planet):
        return (
            df.dropna(subset=[planet])
            .groupby("Last Name")[planet]
            .agg(lambda x: x.mode()[0] if not x.empty else None)
            .to_dict()
        )
    ...
    for planet in HomePlanet_Features:
        mask = df[planet] == 1
        df.loc[mask, "HomePlanet"] = df.loc[mask, "HomePlanet"].fillna(
            planet.split("_", 1)[1]
        )

    deck_mask = (df[deck_Features].isna().sum(axis=1) == 1) & (
        (df[deck_Features] == 0).sum(axis=1) == len(deck_Features) - 1
    )
...
def Impute_df(
    ...
    if impute_method == "flag":
        ...
        recalc_cols = [col for col in recalc_cols0 if col not in columns_to_drop]
        if recalc_cols:
    ...
    int_features = list(
        {
            f
            for f in df_all.select_dtypes(include="number").columns
            if df_all[f].dropna().mod(1).abs().lt(eps).all()
        }
        - set(columns_to_drop)
    )
    bool_features = list(
        {
            f
            for f in df_all.select_dtypes(include="object").columns
            if set(df_all[f].dropna().unique()).issubset({"True", "False", 0, 1})
        }
        - set(columns_to_drop)
        - {"Transported"}
    )
    ...
    df_test = (
        (df_all_imputed.iloc[-len(df_test) :])
...
def Impute_Cabins(df_orig, regions_bin_edges):
    ...
    Group_to_Cabin = (
        ...
        .agg(lambda x: x.mode().iloc[-1] if not x.empty else None)
        .to_dict()
    )
    ...
    if df.Cabin.isna().any():
        null_rows = df[df.Cabin.isna()]
        print(null_rows)
        raise ValueError("Cabin still containing null values after imputation")
...
def FindFill_missing_cabins(df_orig, deck, side, planets_to_mask, nround):
    ...
    def get_mask(group_min, group_max, cond):
        group_mask = (
            (df["GroupId"] >= group_min)
            & (df["GroupId"] <= group_max)
            & (df["Cabin Number"].isnull())
        )
        ...
        if not planets_to_mask:
            ...
            print(
                "error: planets_to_mask dictionary should not have more than one entry"
            )
            return 0
        if cond == 1:
    ...
    for cabin in missing_cabins:
        ...
        if condition1:
            ...
            groups_in_cabin = set(
                [null_cabin_rows.index[k][:4] for k in range(len(null_cabin_rows))]
            )
...
def Impute_Planets(df_orig):
    ...
    HomePlanet_Features = [col for col in df.columns if col.startswith("HomePlanet_")]
    deck_Features = [col for col in df.columns if col.startswith("deck_")]
    HomePlanetDeck_Features = [
        col for col in df.columns if col.startswith("HomePlanetDeck_")
    ]
    Luxury_Features = ["RoomService", "FoodCourt", "ShoppingMall", "Spa", "VRDeck"]

    Imp_Features = HomePlanet_Features + Destination_Features
    Dep_Features = (
        ["CryoSleep", "GroupSize", "Age", "CabinSize", "CabinFamilySize", "FamilySize"]
        + deck_Features
        + Luxury_Features
    )
    High_Dep_Features = deck_Features + HomePlanet_Features

    df = KNN_Impute_Features(
        df, Imp_Features, Dep_Features, High_Dep_Features, n_neighbors=9
    )
    ...
    df["eps"] = np.where(
        df["HomePlanet_Earth"] == 1, np.random.uniform(0.5 * eps, eps), 0
    )
    df["Destination_PSO J318.5-22"] += df["eps"]
    df["eps"] = np.where(
        df["HomePlanet_Earth"] == 0, np.random.uniform(0.5 * eps, eps), 0
    )
    df["Destination_55 Cancri e"] += df["eps"]
    df.drop(columns=["eps"], inplace=True)

    # Now, let's normalize HomePlanet and Destination Features
    df[HomePlanet_Features] = df[HomePlanet_Features].div(
        df[HomePlanet_Features].sum(axis=1), axis=0
    )
    df[Destination_Features] = df[Destination_Features].div(
        df[Destination_Features].sum(axis=1), axis=0
    )

    # Next, pick planet with highest probability and set the rest to false
    df[HomePlanet_Features] = (
        df[HomePlanet_Features]
        .eq(df[HomePlanet_Features].max(axis=1), axis=0)
        .astype(int)
    )
    df[Destination_Features] = (
        df[Destination_Features]
        .eq(df[Destination_Features].max(axis=1), axis=0)
        .astype(int)
    )
    ...
    for planet in HomePlanet_Features:
        mask = df[planet] == 1
        df.loc[mask, "HomePlanet"] = df.loc[mask, "HomePlanet"].fillna(
            planet.split("HomePlanet_")[1]
        )

    for planet in Destination_Features:
        mask = df[planet] == 1
        df.loc[mask, "Destination"] = df.loc[mask, "Destination"].fillna(
            planet.split("Destination_")[1]
        )
...
def Impute_CryoSleep_VIP(df_orig):
    ...
    mask_dest = (df["Destination_55 Cancri e"] == 1) | (
    ...
    Dep_Features = (
        ["GroupSize", "Age", "CabinSize", "CabinFamilySize"]
        + Destination_Features
        + HomePlanet_Features
        + deck_Features
        + Luxury_Features
...
def Impute_Age_Luxury(df_orig):
    ...
    df["Age_LifeStages"] = (
        df.Age_LifeStages_lbs.map(
            {val: idx for idx, val in enumerate(LifeStages_choices)}
        )
    ).astype(int)
...
def KNN_Impute_Features(
    df_orig, Imp_Features, Dep_Features, High_Dep_Features, n_neighbors=5
):
    df = df_orig.copy()
    ...
    df_imputed = pd.DataFrame(
        imputer.fit_transform(df_imputed), columns=df_imputed.columns
    )
    df_imputed[High_Dep_Features] /= 2

    df_imputed = pd.DataFrame(
        scaler.inverse_transform(df_imputed), columns=df_imputed.columns
    )
...
class FeatureImportance:
    def __init__(
        ...
        self.Cat_Features = (
            Binary_Categorical + Ordinal_Categorical + Nominal_Categorical
        )
        self.Nom_Cat_Features = Nominal_Categorical
        assert set(self.Cat_Features).issubset(set(self.X.columns))
        self.ohe_cols = [
            f"{col}_{c}" for col in self.Nom_Cat_Features for c in self.X[col].unique()
        ]
    ...
    def _mi_input(self):
        ...
        enc = OrdinalEncoder()
        X_clean[self.Nom_Cat_Features] = enc.fit_transform(
            X_clean[self.Nom_Cat_Features]
        )
    ...
    def mutual_info(self, random_state=None):
        ...
        MI = mutual_info_classif(
            X_mi, self.y, discrete_features=discrete_mask, random_state=rs
        )
    ...
    def LCV_importance(self):
        ...
        return pd.Series(importance, index=X_lcv.columns.tolist()).sort_values(
            ascending=False
        )

    def catboost_perm_importance_cv(
        self,
        n_splits=5,
        n_repeats=3,
        n_repeats_pi=100,
        ...
        return pd.Series(
            np.vstack(features_importances).mean(axis=0), index=X_clean.columns.tolist()
